...
- **A delete made during a sync is no longer undone.** Removing an entry while a sync was already running let that sync write the entry back, and nothing on screen said so. The sync now checks what the device has recorded as deleted immediately before it writes, and the delete reaches the account on the next sync.
- **The record of deletes waiting to sync no longer grows forever.** Four kinds of entry were written into that record and never removed, including entries created and deleted between two syncs. Every entry a sync has weighed is now cleared once that sync has been agreed with the account.
- **A restore message is only shown to the account it is about.** The message that says your device lost its local copy and your account put it back stays on screen after the sync that produced it. Signing in as somebody else on the same device showed them that message, with the first person's number of entries. The message now carries the account it describes, and it is dropped when a different account signs in.
- **The number of restored entries is now the number of rows that came back.** Two things were counted wrongly. When your account's list of fasts or saved meals replaced the one on your device, nothing was reported at all, because those two lists carry no deletion records. And a sync that held back the sealed part of your data counted that as a restored entry, although nothing was written to the device. The count is now the diary rows the sync actually wrote back, and a held sealed part is written to the log only.
