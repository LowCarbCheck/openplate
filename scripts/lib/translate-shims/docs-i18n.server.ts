 ...
 * `hash` is the KEY of the translation memory. Every entry in `app/i18n/memory/<locale>.json`
 * is filed under `hash(englishLeaf)`, and the vendored `translate-ui.ts` looks a leaf up by the
 * same function. A hash that differed from the website's by one character would not be wrong
 * in any way a test could see: every lookup would miss, every string would be a miss, and the
 * next run would buy the whole catalog again. `Memo`, `Memory` and `Unit` are the file format
 * and the unit of work, and `fits` is the marker check `fill` runs. All five are the upstream
 * text, unchanged; the upstream module's hash is under `shimmed` in TRANSLATE_SOURCE.json.
 *
 ...
 * `collectUnits`, `pagesOf` and `tablesOf`, none of which the UI path touches: this app has no
 * documentation corpus and nothing here constructs a `Block`. A silent no-op would let a future
 * caller collect nothing, price nothing and report a corpus as fully translated. They throw,
 * naming the reason, so that day is an error with a sentence in it.
 */
import { createHash } from 'node:crypto';

import type { Block, DocEntry, DocFile, Inline } from './docs';
 ...
 * the memory sees `"de": "..."` next to the English it stands for. Every value is a string, which
...
const NO_DOCS =
  'translate: the documentation corpus is not part of openplate. This copy of the translator serves ' +
  'app/i18n/locales only; see scripts/lib/translate-shims/docs-i18n.server.ts.';
