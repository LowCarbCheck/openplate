...
The service cannot read a single entry, which also means it cannot recover one: a forgotten
passphrase plus a lost recovery code is unrecoverable data loss. [sync.md](sync.md) states
that trade-off in full, and so does the app before you finish setting sync up.
