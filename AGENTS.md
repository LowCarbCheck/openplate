...
5. **Regenerate the in-app release notes, and buy their five translations.** `pnpm release-catalog` rewrites `app/i18n/locales/en/releases.json` from the section you just renamed: the newest three releases that changed the application, with the bold lead of each Added, Changed and Fixed bullet. Then buy the translations, one locale at a time, for `de`, `fr`, `it`, `es` and `tr`:
   ...
   The key is `OPENROUTER_API_KEY` in the workspace root `.env`. Thread it through `env` as above and never print it. Each run rewrites every catalog for that locale, so **stage the six `app/i18n/locales/*/releases.json` files and the five `app/i18n/memory/*.json` files** in this same release commit. `tests/unit/release-catalog.test.ts` compares the committed English catalog against `CHANGELOG.md`, so a forgotten run fails the pre-push gate instead of shipping a card about a version that is no longer the newest. `pnpm release-catalog --check` asks the same question without writing anything. **Then read what the translator did with the names of screens.** Every run now carries a screen glossary (`scripts/translate-glossary.ts`), built at run time from the English and the target `common.json`, so the model is given the app's own name for each screen in the language it is writing, and the run prints how many of the nine it could name. Every run also prints a report of the remembered release leads whose English names a screen and whose translation answers with another word, each with its memory hash. It is a report and never a failure, because a good translation may leave the noun out. Read it, and read the leads yourself as well: one English name covers two screens ("Overview" is the home screen and the first Insights tab), and only a person can tell which one a lead meant. To change one, either delete its hash from `app/i18n/memory/<locale>.json` and run that locale again to buy it, or correct the entry by hand and set its `model` to `hand-written`, then run that locale again to rebuild the catalog. See [ADR-0018](.adr/0018-in-app-release-notes-come-from-the-changelog.md) for why this is a generated file and not a fetch.
