...
- **Joining an operator-run gateway is still BYOK, not an exception to it.** A user may follow an invite link (`/join`, `app/lib/join-link.ts`, `app/lib/gateway-invite.ts`; `/connect-gateway` still resolves and redirects into it) to join a household/organization gateway someone else runs. The call stays browser → gateway direct — this app's server never sees the invite token, the member token or a plate photo, exactly as above. What the invite flow writes is an ordinary `openai-compatible` settings row, distinguished only by `connectedVia: 'invite'`; there is no new provider concept and no server state. One thing genuinely changes: a gateway may declare `auditEnabled`, meaning _that gateway_ — not this server — stores submissions for its operator. The client discloses this wherever it's relevant (the join confirm card, AI settings, the scan surface), and a gateway flipping its audit mode after the fact surfaces to the user as the `reconsent-required` failure cause (`app/services/vision/failure-cause.ts`), not as a generic auth error. This is a scoping of the hard rules above, not a weakening of them.
