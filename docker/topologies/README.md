...
| 2 | App + sync | [`compose.sync.yml`](compose.sync.yml) | End-to-end-encrypted sync between a person's devices, and accounts to hang it on. The server cannot read the entries. | Three containers, a Postgres, and `SERVER_SECRET` (back it up *with* the database). No mail: an account is a handle plus a passphrase. |
