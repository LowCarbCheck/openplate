 ...
 * | `/diary`     |       8 | the two, the active tab (two), the strip's photo button, two links, the habit day's dot. It was 9 until the lead figure (layout D, 2026-09-23) dropped the budget row's status dot. The photo button took the filled camera key's place in M260 and costs the same one: its fill is teal, its glyph and label are not |
 * | `/dashboard` |      14 | the two, the strip's photo button (the camera key until M260, the same one), two hand-off links (four), the award mark, four data marks (the status dot, an adherence cell, the streak legend dot and a ridge bar), and the bar's lit More tab (two). It was 12 until M258: Overview is a More tile, and the playground the operator chose from lights More on the pages it holds, so a person on Overview sees where they are |
 *
