...
describe('getInstanceInferencePreset', () => {
  it('is null for every shape of "no preset" — no UI may render on any of them', () => {
    ...
    assert.equal(getInstanceInferencePreset({ syncServerUrl: null, instancePreset: null }), null);
  });

  it('hands the configured preset through untouched', () => {
    assert.deepEqual(getInstanceInferencePreset({ syncServerUrl: null, instancePreset: PRESET }), PRESET);
  });
});
