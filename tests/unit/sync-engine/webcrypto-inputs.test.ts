...
import { generateDek, unwrapDek, wrapDek } from '../../../app/lib/sync/engine/crypto/dek-wrap';
...
test('an envelope round-trips when both the DEK and the AAD come from exotic buffers', async () => {
  const dekSource = wasmMemoryView({ shared: true });
  const dek = new Uint8Array(dekSource);
  const aadFields = { accountId: 7, blobVersion: 3, payloadSchemaVersion: 6 };
  const payload = { snapshot: { hello: 'world' }, syncMeta: { perEntity: {}, tombstones: [] } };

  const envelope = await buildEnvelope({ payload, dek: dekSource, aadFields });
  const parsed = await parseEnvelope({ envelope, dek, aadFields });

  assert.deepEqual(parsed, payload);
});

test('the whole envelope path hands WebCrypto only safe BufferSources', async () => {
  const dek = wasmMemoryView({ shared: false });
  const aadFields = { accountId: 1, blobVersion: 1, payloadSchemaVersion: 6 };
  const { calls } = await captureSubtleCalls(async () => {
    const envelope = await buildEnvelope({
      payload: { snapshot: { a: 1 }, syncMeta: { perEntity: {}, tombstones: [] } },
      dek,
      aadFields,
    });
    return parseEnvelope({ envelope, dek, aadFields });
  });

  assert.ok(calls.length >= 2, 'expected both an encrypt and a decrypt');
  for (const call of calls) {
    const params = aesParams(call);
    assertSafeBufferSource(params.iv, `${call.operation} iv`);
    assertSafeBufferSource(params.additionalData, `${call.operation} additionalData`);
