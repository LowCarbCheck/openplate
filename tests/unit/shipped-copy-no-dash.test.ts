 ...
 * applied to these bundles. Nothing enforced it: a 2026-09 pass removed
 * every em dash (U+2014) and en dash (U+2013) from the English and German
 * bundles by hand, and one of them had already drifted back into a test
 * assertion before this file existed. The two legal bundles it also covered
 * left the repository in M246; the legal text is now in mounted content files
 * that this repository does not hold. `managed-copy-bans.test.ts` checks the same two
 * characters, but only across the managed-instance branch of `common.json`;
 * this file is the repo-wide backstop over the SHIPPED bundles, so a
