 ...
 *    "openplate". `tests/unit/wordmark-only-serif.test.ts` holds the same rule at the source;
 *    this is the read that sees a rule in a stylesheet or an inherited family the source does
 *    not show.
 *
 ...
 * nothing. The wordmark sweep is likewise pointed at a forced-Fraunces heading it must flag.
 *
 * A DIARY AND NOT AN EMPTY ONE. See above. The food is logged through the real manual form.
 */
import { expect, test, type Page } from '@playwright/test';

import { CARD_TITLE_PX, FRAUNCES, VICTOR_MONO, familyStartsWith } from '../design-contract';
...
/**
 * Screens swept for Fraunces before onboarding: the landing page, where the wordmark is largest
...
/** Screens swept for Fraunces after onboarding. The header kicker is the wordmark on each. */
const PERSONAL_WORDMARK_ROUTES = ['/diary', '/dashboard', '/settings', '/trends'] as const;

/** What every serif element on a screen must read as. */
...
test(`a card title is ${CARD_TITLE_PX}px in Victor Mono, and the reader can say no`, async ({ page }) => {
  ...
  expect(forced.family, 'the control must be in the serif').not.toMatch(familyStartsWith(VICTOR_MONO));
  expect(forced.size, 'the control must be at the old size').not.toBe(`${CARD_TITLE_PX}px`);
});

/**
 * The text of every element on the page whose computed family is Fraunces.
 *
 * @param page - a page on the production build.
 * @returns each serif element's trimmed text.
 */
async function serifTexts(page: Page): Promise<string[]> {
...
test('Fraunces draws the word openplate and nothing else, and the sweep can say no', async ({ page }) => {
  ...
  for (const route of PUBLIC_WORDMARK_ROUTES) {
    ...
    expect(
      (await serifTexts(page)).filter((text) => text !== WORDMARK_TEXT),
      `${route}: something other than the word "${WORDMARK_TEXT}" is drawn in the serif`,
    ).toEqual([]);
  }
  ...
  expect(await serifTexts(page), 'the landing heading is the wordmark').toContain(WORDMARK_TEXT);
  ...
  for (const route of PERSONAL_WORDMARK_ROUTES) {
    ...
    const texts = await serifTexts(page);
  ...
  // CONTROL (b): the sweep flags a card title forced into the serif.
  await page.evaluate(
    ({ family }) => {
      const heading = document.createElement('h2');
      heading.textContent = 'A card title in the serif';
      heading.style.fontFamily = `"${family}", serif`;
      document.body.append(heading);
    },
    { family: FRAUNCES },
  );
  expect(await serifTexts(page), 'the sweep must flag a serif that is not the wordmark').toContain(
    'A card title in the serif',
  );
});

test('the header page title is in the body face while the kicker above it is the wordmark', async ({ page }) => {
  await completeOnboarding(page);
  ...
  await expect
    .poll(() => kicker.evaluate((element) => getComputedStyle(element).fontFamily), { message: 'the header kicker face' })
    .toMatch(familyStartsWith(FRAUNCES));
});
