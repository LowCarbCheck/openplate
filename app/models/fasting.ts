 ...
 * in the brand role (DESIGN.md §4): `font-display` is the wordmark's, and the
 * `Wordmark` component sets it at weight 100, which is no weight for a number
 * that changes once a second. The brand role draws the product's own name and
 * nothing else, so this is a guard against a reintroduction rather than a live
 * risk, and `tabular-nums` stays even though the body face is tabular by
 * construction.
 */
