/**
 * IndexedDB persister wiring for the primary, outbox, photos, and AI-settings
 * stores. The stores are singletons created on first access; each is kept in
 * sync with `startAutoLoad` (loads on start, then watches for OTHER tabs'
 * writes → this tab, every tab, always) and `startLockedAutoSave` (this tab's
 * OWN writes → IndexedDB, every tab, serialized via a Web Lock scoped to each
 * individual save — see mechanism 2 below).
 *
 ...
 * `createOutboxStore()` / ...) and pass it explicitly via the `{ store }`
 * option every local-store function accepts, rather than calling a `getXStore`
 * singleton.
 *
 * FIVE SAFETY MECHANISMS live in this file, all added after confirmed
 * data-loss or looping-error incidents:
 *
 ...
 *    nothing while `v`, the store-level values, survived untouched;
 *    `diary`/`add`/etc. then read zero rows and treated it as a brand new
 *    device. An in-memory store that comes back EMPTY after `startAutoLoad()`
 *    is only trustworthy as "this device has no data" when the persisted
 *    IndexedDB is *also* empty. If IndexedDB actually holds rows and none of
 *    them made it into the store, that is a failed or partial load, not an
 *    empty device — this function refuses to let the caller proceed to
 *    autosaving, which would otherwise overwrite the persisted rows with
 *    nothing. See its own doc for the mechanics.
 ...
 *    never persisted anything — so a user logging a meal in a second tab
 ...
 *    page actually going away, so a user who logs a food and immediately
 *    closes the tab (or backgrounds the app) can have their write reach the
 *    in-memory store — and be told "Added X — nice." — before the async
 ...
 *    from the four below: `createIndexedDbPersister`'s `load()` (used by
 ...
 *    On a device where a given store (photos, in the incident that surfaced
 *    this — but the mechanism is generic to every `getXStore()` singleton
 *    below, since they all share this one `initPersistedStore`) has never
 *    saved anything, the versionless load-open creates an EMPTY v1 database
 *    with NO object stores at all; the very next `transaction(['t','v'])`
 *    throws `NotFoundError`, and `startAutoLoad`'s ~1s poll (see
 *    `autoLoadIntervalSeconds`) repeats that failure forever — this is what a
 *    device would otherwise self-heal from only the moment SOMETHING first
 *    saves. `primeFreshDatabaseIfNeeded` runs a one-time, empty
 *    `persister.save()` before `startAutoLoad` ever gets a chance to poll,
 *    which is enough to trigger the v2 upgrade and create both object stores
 *    up front. An empty save can never clobber data — see
 *    `shouldPrimePersistedDb`'s doc for why `readPersistedTableRowCounts`
 *    resolving `null` proves there was nothing to clobber.
 ...
 *    intermittent failure in `tests/unit/local-store-cross-tab.test.ts` (see
 *    that file's flake post-mortem), and was then reproduced deterministically
 *    against real `tinybase@9.2.0`. TWO of its behaviours conspire whenever a
 ...
 *    away (see `recordChangesDuringLoad` for how the load's OWN replace is
 *    told apart from the user's writes). `runLockedSave` waits out any
 *    in-flight load before calling `save()`, so the save that persists that
 *    restoration — and, just as importantly, a `pagehide` flush that happens
 *    to land during a poll — is not itself silently discarded by (a).
 *    See `startLockedAutoSave`'s doc for the windows this still does NOT
 *    close.
 ...
 *    user-visible defect: a food logged from `/add` showed its success toast
 *    and was simply gone, in memory and on disk, immediately afterwards
 *    (openplate#1).
 *
 ...
 *    that in practice lasts as long as the main thread is busy (after an add,
 ...
 *    that no longer contains the write. Reproduced deterministically in
 *    `tests/unit/persist-load-guard.test.ts`.
 *
...
function assertBrowserWithIndexedDb(dbName: string): void {
  ...
  throw new Error(
    `local-store is browser-only: getXStore("${dbName}") was called outside a browser with IndexedDB support. ` +
      ...
      '(createPrimaryStore()/createOutboxStore()/createPhotosStore()/createAiStore()) and pass an explicit ' +
 ...
 * so the primary health data survives storage pressure. Requested lazily from
 * the primary-store WRITE path (see `primary-store.ts`) on the first tracker
 * write — NOT gated on PWA install, so a user who never installs still gets the
 * durability request the moment they log anything. Fire-and-forget and
 * single-shot: the browser only prompts (if at all) once per origin, and an
 * already-persisted origin resolves `true` immediately. Inert under SSR/Node
 * (no `navigator`).
 */
...
/**
 * The object store name `tinybase/persisters/persister-indexed-db` (v9.2.0,
 * verified against its source) uses to hold table content — one row per
 * table, `{k: tableId, v: tableContent}`. This is an implementation detail of
 * the library, not a published API, but it's the only way to read "does
 * IndexedDB actually have data" without going through the very
 * `Store`/`Persister` load path this module exists to verify.
 */
 ...
 * replicate; widening the return type to `Promise<unknown>` keeps a real
 * `IndexedDbPersister` assignable here (its `Promise<IndexedDbPersister>` is a
 * subtype) while letting tests pass a plain `{ startAutoLoad: async () => {} }`.
 */
interface LoadStep {
 ...
 *
 * Row COUNTS (never row CONTENTS — no food names, no macros, no weight
 * numbers) are logged on every call, success or refusal, so a future
 * occurrence is diagnosable from log aggregation even though the failure
...
const SAVE_LOCK_PREFIX = 'openplate-local-store-save:';
 ...
 * 1 = Loading, 2 = Saving — verified against `tinybase@9.2.0`'s
 * `persisters/index.js`, and the same constant the cross-tab test spells out
 * for the same reason.
 */
 ...
 * {@link LoadStep}'s doc for why this isn't `Pick<IndexedDbPersister,
 * 'save'>` directly (the real `save(): Promise<this>` return type doesn't
 * structurally match a `Promise<void>`-returning test double).
 *
 ...
 * a double, and every mechanism-4 code path is then inert rather than guessing.
 */
interface SaveStep {
...
/**
 * The persister-load seam mechanisms 2 and 4 both need, reduced to the two
 ...
 * (in which case both mechanism-4 code paths no-op — see {@link SaveStep}).
 */
 ...
 * the previous `becomeAutoSaveWriter`, which held its lock for an elected
 * tab's ENTIRE lifetime and only ever called save from that one tab — see
 * this file's module doc for the data-loss incident that caused.
 *
 ...
 * the easy way to avoid ever resurrecting a row, but it would also mean a food
 * log the user deleted could reappear after the next poll — a worse bug than
 * the one this closes.
 */
 ...
 * so nothing else can be recorded after it. Verified empirically against
 * `tinybase@9.2.0`, not just read off the source.
 *
 ...
 * — a burst of several local writes (e.g. logging three foods back to back)
 ...
 * cause of openplate#1 (a logged food vanishing behind its own success toast).
...
export function startLockedAutoSave(
  ...
  const listenerId = store.addDidFinishTransactionListener((changedStore) => {
    ...
    if (restoringPending) {
      // Counts only — never cell contents, which are food names, macros and
      // weights. `warn`, because reaching here means the restoration above
      // genuinely CHANGED something: a local write really had been erased from
      // the store the UI reads. The overwhelmingly common no-op restoration
      // never fires this listener at all.
      log.warn(
 ...
 * Safari/iOS home-screen PWAs, this app's primary target) and on
 * `visibilitychange` when `document.visibilityState` becomes `'hidden'`
 * (covers app-switching/backgrounding on mobile, which does not always fire
 * `pagehide` first). `beforeunload` is deliberately NOT used: it is
 * unreliable on mobile Safari, and on desktop merely registering a listener
 * for it disables the page's back/forward cache.
 *
 ...
 * cache-eviction path can ever touch it. Throws if resolved outside a browser
 ...
 * case the failed promise is NOT cached, so the next call retries from
 * scratch rather than being stuck permanently rejected.
 */
...
/**
 * The lazily-created, IndexedDB-backed on-device photo store. Its own
 * IndexedDB database, wholly independent of the primary and outbox stores —
 * clearing or evicting photos can never disturb tracker data. Throws if resolved
 * outside a browser with IndexedDB support — see `assertBrowserWithIndexedDb`
 * (every public caller in `photos.ts` wraps this in a try/catch, so a thrown
 * guard is swallowed the same as any other best-effort photo-cache failure).
 * Also throws (uncached, so a later call retries) if a suspected failed/partial
 * load was detected — see `loadAndVerifyOrThrow`.
 */
export function getPhotosStore(): Promise<Store> {
  if (!photosPromise) {
    photosPromise = initPersistedStore(createPhotosStore(), PHOTOS_DB_NAME).catch((cause: unknown) => {
      photosPromise = null;
      throw cause;
    });
  }
...
/**
 * The lazily-created, IndexedDB-backed BYOK AI-settings + local-usage store
 * (M117/02). Its own IndexedDB database — wholly independent of the primary
 * store, so `backup.ts`'s export envelope can never carry a user's API key.
 * Throws if resolved outside a browser with IndexedDB support — see
 * `assertBrowserWithIndexedDb` (every public caller in `ai-settings.ts` /
 * `ai-usage-log.ts` accepts an explicit `{ store }` override for tests). Also
 * throws (uncached, so a later call retries) if a suspected failed/partial
 * load was detected — see `loadAndVerifyOrThrow`.
 */
