...
const EMPTY_PROFILE_GOALS: LocalProfileGoals = {
  ...
  eatingStyle: null,
  // Added within v24 (the main goal). `null` is "never picked", which
  // `effectiveMainGoal` reads as the lens's own lead, so a device that has
  // never seen the setting shows the card it always showed.
  mainGoal: null,
