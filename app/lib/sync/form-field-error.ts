 ...
 * half-completed setup impossible. But two of the service's refusals are about
 * a field the person can still see and fix — "that sign-in name is taken" and
 * "that invite is not valid" — and a message about one field, rendered under
 * the submit button, is the shape the owner asked us to stop producing
 * (2026-09-02).
 *
 ...
 *
 * The message is ALREADY TRANSLATED copy: the thrower is the surface that knows
 * both the service's status code and this instance's signup mode, and
 * `PROTOCOL.md` §4 is explicit that a client branches on the status rather than
 * on the service's own English prose.
 */

/** The signup fields a service refusal can be attributed to. */
export type SyncFormField = 'handle' | 'invite';
 ...
 * service's statuses to copy (`settings.sync.tsx`'s `describeSignupError`) has
 * four branches, and this is the contract they all owe.
 */
