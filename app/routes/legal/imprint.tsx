/**
 * `/imprint`: the `imprint` page of the mounted content folder (M246 spec 01).
 *
 * The text is not in this repository. The loader reads
 * `<CONTENT_DIR>/<lang>/imprint.md` for this request's language (English when
...
export async function loader({ request }: Route.LoaderArgs) {
  return { page: await loadContentPageOrThrow({ request, slug: 'imprint' }) };
}

export const meta: Route.MetaFunction = ({ loaderData }) => [{ title: contentPageTitle(loaderData?.page.title ?? null) }];

export default function Imprint({ loaderData }: Route.ComponentProps) {
