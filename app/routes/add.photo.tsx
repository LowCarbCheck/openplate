...
/**
 * The not-connected card, and the loading placeholder it shares with this
 * route's `HydrateFallback`, both LIFTED into `components/intake/` (M233/02)
 * because `/pantry` reaches the same dead end for the same reasons.
 *
 * RE-EXPORTED rather than merely moved: `tests/unit/scan-connect-card.test.ts`
 * and `scan-signed-out-card.test.ts` render each shape through this module,
 * and a move that also changed where they import from would mix a pure
 * relocation with a test edit in one diff.
 */
export {
  ConnectCard,
  ConnectCardView,
...
} from '#app/components/intake/intake-connect-card';
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-4">
            ...
            {error && (
              <IntakeFailureAlert subject={isTextIntake ? 'text' : 'photo'} title={alertTitle}>
                {
