 ...
 * The obvious shortcut — importing `#app/i18n/i18n` and calling
 * `i18next.t(...)` — is a real bug on the server, because that singleton is
 * one process-wide instance shared by every in-flight request. Whichever
 * language the last request happened to set is the language the next request's
 * `<title>` would be rendered in. Under any concurrency at all, that serves one
 ...
 * state, nothing to leak between requests — two callers on two requests with
 ...
 *   { title: metaTitle(metaLanguage(matches), 'meta.diary') },
 * ];
 * ```
 */
import { z } from 'zod';
 ...
 * boundary can run `meta()` with no loader data at all, so anything that isn't
...
/** Narrows any language-ish value to a supported code — mirrors `date-locale.ts`. */
 ...
 * not what we expect — an error boundary can render `meta()` with no loader
 * data at all, and the language ultimately originates in a non-httpOnly
 * cookie, so a bad value must degrade to an English title rather than throw
 * and blank the document head.
 *
