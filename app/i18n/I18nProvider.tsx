 ...
 *
 * `i18n` (./i18n.ts) is a MODULE-SCOPED SINGLETON — one per Node process,
 * shared by every concurrent request that process serves. Calling
 ...
 * handled differently:
 *
 *   - SERVER: each render gets its own `i18n.cloneInstance({ lng })`. The
 *     clone shares the already-parsed resource store (no re-parsing the locale
 *     JSON per request) but owns its `language`/`resolvedLanguage`, so nothing
 *     it does is visible to another request or to the singleton. Passing `lng`
 *     explicitly also means the clone's detector never runs — moot on the
 *     server anyway, where there is no `document`.
 *
 *   - CLIENT: one browser tab per instance, so mutating the singleton is safe.
 *     It is synced INLINE during render, not in a `useEffect` — an effect runs
 *     a tick too late, after a mismatched first paint has already committed.
 *     `changeLanguage()` resolves synchronously because both bundles are
 *     inline ESM imports.
 *
