 ...
 * a place here.
 *
 * One member is NOT an address: `managed`. It exists because the shape of the
 * product follows from it — a managed instance has one door, no anonymous
 * diary, and AI that comes from its own server. Deriving it in each screen
 * instead would let "this is a managed instance" be true on one and false on
 * the next.
 *
 * M192 removed `gatewayUrl`. There is no second service to dial: the sync
 * server IS the AI proxy, so the address the browser already has is the only
...
export interface PublicConfig {
  ...
  /**
   * Whether this instance is a MANAGED instance: one an organization runs for
   * its people, where an admin invites by email and the server supplies the
   * AI.
   *
   * `false` is the DEFAULT and the self-host default, and it is the app in
   * full: `/welcome` offers "Start", `/onboarding` opens an anonymous
   * local-only diary, and AI is the person's own provider key. All three are
   * right when somebody can genuinely use the app without an account.
   *
   * `true` closes that anonymous path, because on a managed instance it leads
   * nowhere: there is no AI without the account, and no account without an
   * invite. It is the operator's own declaration (`INSTANCE_MODE=managed`)
   * rather than an inference from some other value being set — see
...
/**
 * The two shapes an instance can have.
 *
 * `'open'` is the self-host default: anyone can start a local diary, sync is
 * an optional extra, and AI is the person's own provider key.
 *
 * `'managed'` is an organization's instance: an admin invites people by email,
 * every account comes from an invite, and the AI comes from the server on the
 * account's own allowance.
 */
export const INSTANCE_MODES = ['open', 'managed'] as const;
export type InstanceMode = (typeof INSTANCE_MODES)[number];

/**
 * Parses `INSTANCE_MODE`.
 *
 * - unset, empty, or whitespace → `'open'` (the default and the self-host
 *   default)
 * - `'open'` or `'managed'` → itself
 * - anything else → THROWS
 *
 * The throw is the point, and it is the same argument the deleted
 * `parseGatewayUrl` made: an operator who typed `INSTANCE_MODE=manged` wants a
 * managed instance. Falling back to `'open'` would re-open the front door of a
 * closed organization's app without touching anything they can see.
 */
export function parseInstanceMode(raw: string | undefined): InstanceMode {
  const value = raw?.trim().toLowerCase();
  if (value === undefined || value === '') return 'open';
  if (value !== 'open' && value !== 'managed') {
    throw new Error(
      `Invalid INSTANCE_MODE: expected one of ${INSTANCE_MODES.join('/')}, got "${raw}". ` +
        'It is refused rather than ignored because ignoring it would silently run an ' +
        "organization's instance as an open one.",
    );
  }
  return value;
}
...
/**
 * Refuses `GATEWAY_URL`, which M192 deleted along with the service it named.
 *
 * A REFUSAL RATHER THAN SILENCE, because the variable used to be what MADE an
 * instance managed. An operator who upgrades without editing their environment
 * would otherwise get an open instance — anonymous diaries offered to
 * strangers — from a configuration file that still says "gateway" and looks
 * untouched. Naming the replacement in the message is what turns a stopped
 * boot into a one-line fix.
 *
 * @throws when `GATEWAY_URL` is set to anything at all.
 */
 ...
 * `managed` with no server address is not a managed instance, it is one whose
 * every screen would be a dead end — and the operator would find out from
 * their users.
 *
 * @throws when `instanceMode` is `'managed'` and `syncServerUrl` is `null`.
 */
