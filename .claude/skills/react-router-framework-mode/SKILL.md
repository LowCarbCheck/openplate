...
Framework mode is React Router's full-stack development experience with file-based routing, server-side/client-side/static rendering strategies, data loading and mutations, and a type-safe route module API. This is the mode ts-factory-stack runs (SSR via Express in `server.ts`).
...
ts-factory-stack targets **React Router v8**. Verify the installed version before implementing:
