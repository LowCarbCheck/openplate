...
- **Status:** Superseded (2026-08-19) — the runner, its `data_migrations` ledger, the CLI that invoked it, and the whole Postgres dependency were removed. The server persists nothing, so there is nothing left to migrate; device-side data changes are handled by the IndexedDB store's own versioning in `app/lib/local-store/`.
- **Date:** 2026-05-13
- **Deciders:** Altan
